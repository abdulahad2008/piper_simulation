...
from a console log. Phase~0 (Sec.~\ref{sec:phase0}) is \emph{complete}: it
required no training, only the released checkpoints. Phase~1 and Phase~2
require 43 training runs that have not been run; everything describing them is
...
pushing the success tolerance below a few millimetres collapses success
regardless of how much longer one trains. \citet{xu2026curse} formalise this
floor as a limit precision $c$ in a data-scaling law for behaviour cloning and
describe $c$ as ``an emergent property of the entire agent system,'' leaving
open both what determines it and whether it survives online reinforcement
learning. We study $c$ for a reinforcement-learned pick-and-place policy on a
...
randomisation, and run four control arms designed to falsify the interface
hypothesis. This version reports the complete eval-only phase. Two released
...
$c_{50}$ by \input{generated/tables/inline_gate_span}\unskip. \planned{The interface
sweep, the estimated $c(\iface)$ and the training-based control arms are
pending; see Sec.~\ref{sec:results}.} We release the environment, the trained
...
A learned manipulation policy that succeeds on 94.9\% of pick-and-place
episodes when ``success'' means landing within 45\,mm of the target succeeds on
20.8\% of them when the tolerance is 10\,mm and on 5.3\% when it is 5\,mm
($n{=}2000$; Table~\ref{tab:baselines}). Nothing about the policy changed; only
the ruler did. Every practitioner who has tightened a tolerance has seen this
cliff, and yet the community's benchmarks report a single success rate at a
single, usually generous, tolerance \citep{jiang2026benchmarking}, which hides
the cliff entirely.
...
Two features of our setup make the question answerable on one workstation.
...
decomposition over the randomisation space (Sec.~\ref{sec:attrib}).
...
precision-versus-training-steps curve over every stored checkpoint of the
baseline run.
...
tested rather than assumed away. High-precision RL results exist ---
\citet{brown2026match} reach sub-millimetre-clearance insertion with a hybrid
position/force interface at 15\,Hz --- but there the interface is a design
contribution, not a swept variable.
...
assume 6\,h per training run. Phase~0 is complete; Phases~1--2 are pending.}
\label{tab:design}
...
0 & Checkpoint curve, eval-only & every stored checkpoint of both runs & $\hat c$ vs.\ training steps; H0b & 0 & done \\
...
\subsubsection{H0b, partially: precision versus training budget}
\label{sec:ckpt}

\planned{Every stored checkpoint of both runs (32 of \texttt{sac\_full} spanning
50\,k--1.62\,M steps, 6 of \texttt{corner\_ft}) is being evaluated at $n{=}1000$
to give $c_{50}$ as a function of environment steps: the RL analogue of Xu et
al.'s data-scaling curve, and the cheapest available test of H0b. The figure and
the fit of $\log N \propto 1/(P-\hat c)$ with environment steps in place of
demonstrations go here.}
\todo{Fill from \texttt{paper/generated/figures/checkpoints.pdf} and
\texttt{summary.json:checkpoint\_curve} once the sweep finishes. Note that this
is a within-run budget test on one seed and does not replace the $3\times$
budget arm, which is trained from scratch.}
...
\subsection{Failure attribution, Phase 0 cells}
\label{sec:attrib-results}

The per-episode randomisation vector is logged for every episode, so this
analysis is available today for the Phase~0 cells and will be available for each
new cell as it lands. At $\tau = c_{50}$ --- where by construction half the
episodes fail --- a five-fold cross-validated classifier of failure on the
thirteen sampled randomisation and scene variables reaches an AUC of
$0.65$ (logistic) and $0.61$ (gradient boosting) for $\pi_{v1}$, $0.56$ and
$0.59$ for $\pi_{v2}$, and $0.58$ and $0.56$ for the softest solver cell. At
$\tau{=}45$\,mm, where failure means an outright miss rather than an imprecise
placement, the AUCs are $0.52$--$0.63$. Permutation importance puts the target's
position in the workspace on top in every cell --- target radius and azimuth for
$\pi_{v1}$, target azimuth and object width for $\pi_{v2}$ --- and the object's
mass, friction and the actuator gain scales at or below zero.

The reading is that the residual failures at the floor are \emph{almost}
irreducible under this randomisation. What structure exists is geometric: where
in the reachable workspace the object has to be put, not what it weighs or how
slippery it is or how the actuators were detuned. That is consistent with a
floor set by the terminal approach rather than by the dynamics, and it is a
weaker conclusion than either extreme --- the classifier does beat chance, so
the pre-specified rule ``AUC $<0.6$ and ICC $<0.1$ in every cell, therefore the
failures are noise'' is not met outright at $\tau = c_{50}$ for $\pi_{v1}$.

\planned{The intra-class correlation is estimated from a nested evaluation of
$\pi_{v2}$ --- 200 randomisation draws replayed under 10 independent noise
streams each --- which separates the variance the scene explains from the
variance the noise explains. The environment supports the split
(\texttt{options=\{"dr\_seed": k, "noise\_seed": m\}}) and the run is in
progress.} \todo{Fill the ICC from
\texttt{paper/generated/failure\_attribution.json}, and state the joint verdict
against the pre-specified rule.}
...
All experiments are in simulation on one arm, one task and one algorithm. The
solver arm bounds the dependence of $c$ on the contact parameters we sweep, not
on the contact model as such; it also found a non-zero dependence, which we
report rather than round to nothing. A real-arm measurement remains the only
complete answer, and we do not have one. \todo{If the two-servo encoder rig is
...
\texttt{config.json}. \texttt{tests/test\_interface\_flags.py} and
\texttt{tests/test\_tolerance\_semantics.py} assert that every swept flag
reaches the simulator and that the tolerance means what this paper says it
means. \texttt{CHANGELOG.md} documents every claim that changed between the
